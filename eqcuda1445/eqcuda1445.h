...
int equihash_solve_c(const char *header, uint32_t header_len,
                     uint32_t nonce,
                     void (*on_solution_found)(void *user_data, const unsigned char solution[100]),
                     void *user_data);

inline int EquihashSolveCuda(const void *header, uint32_t header_len, uint32_t nonce, void *user_data) {
